...
//	voltage_limit = new_voltage_limit < voltage_limit ? new_voltage_limit : voltage_limit;
	if(voltage_sensor_align > voltage_limit) voltage_sensor_align = voltage_limit;//否
...
/**************************校准传感器****************************************************/
//校准传感器，获取电机的一些信息极对数，是转子位置位于指定的位置。
int alignSensor(void)
{
	long i;
	float angle;
	...
	for(i=0; i<=500; i++)//第一步：正向转动一个电角周期2pi，和实际的角度无关，分为500步进
	{
		angle = _3PI_2 + _2PI * i / 500.0;
		setPhaseVoltage(voltage_sensor_align, 0,  angle);//voltage_sensor_align为Uq的转动力矩大小
		delay_ms(2);
	}
	mid_angle=getAngle();
	
	for(i=500; i>=0; i--) //第二步：反向转动一个电角周期2pi
	{
		angle = _3PI_2 + _2PI * i / 500.0 ;
	...
	end_angle=getAngle();
	setPhaseVoltage(0, 0, 0);//停止转动
	delay_ms(200);
	printf("delt_angle=%.4f\r\n",mid_angle-end_angle);
//	printf("mid_angle=%.4f\r\n",mid_angle);
//	printf("end_angle=%.4f\r\n",end_angle);
	
	moved =  fabs(mid_angle - end_angle);//一个电周期对应的角度
...
//		printf("fail - estimated pp:");
	...
	delay_ms(500);
	zero_electric_angle = _normalizeAngle(_electricalAngle(sensor_direction*getAngle(), pole_pairs));//我觉着zero_electric_angle没啥用
	delay_ms(20);
...
void move(float new_target)
{
	...
	switch(controller)//controller=0运动控制为转矩模式
	{
		///////////////////////////////////////////////////////
		case Type_torque://转矩模式为电压控制,不带pid
...
// pengqing:逆Park变换：从 Vd 和 Vq 转换到 U_alpha 和 U_beta
void inverse_park_transform(float Vd, float Vq, float angle, float *U_alpha, float *U_beta) 
{
    *U_alpha = Vd * cos(angle) - Vq * sin(angle);
    *U_beta = Vd * sin(angle) + Vq * cos(angle);
}
...
void setPhaseVoltage(float Uq, float Ud, float angle_el)
{
	...
	///////////////////////////////////////////////////////
	if(Uout> 0.3)Uout= 0.3;
	if(Uout<-0.3)Uout=-0.3;

	sector = (angle_el / _PI_3) + 1;
	T1 = 2/_SQRT3*_sin(sector*_PI_3 - angle_el) * Uout;//扇区起始矢量时间
	T2 = 2/_SQRT3*_sin(angle_el - (sector-1.0)*_PI_3) * Uout;//扇区结束矢量时间
	T0 = 1 - T1 - T2;
