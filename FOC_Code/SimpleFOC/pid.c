...
************************************************/
/******************************************************************************/
float pid_vel_P, pid_ang_P,pid_current_P;
float pid_vel_I, pid_ang_D,pid_current_I;
float integral_vel_prev,integral_current_prev;
float error_vel_prev, error_ang_prev,error_current_prev;
float output_vel_ramp,output_current_ramp;
float output_vel_prev,output_current_prev;
unsigned long pid_vel_timestamp, pid_ang_timestamp,pid_current_timestamp;
/******************************************************************************/
void PID_init(void)
{
	pid_vel_P=0.1;  //0.1
	pid_vel_I=5;    //2
	output_vel_ramp=100;       //output derivative limit [volts/second]输出导数限制
	integral_vel_prev=0;
	error_vel_prev=0;
	output_vel_prev=0;
	...
	pid_current_P=0.05;
	pid_current_I=0.01;
	error_current_prev=0;
	pid_current_timestamp=SysTick->VAL;
...
float PID_current(float error)
{
	...
	now_us = SysTick->VAL;
	if(now_us<pid_current_timestamp)Ts = (float)(pid_current_timestamp - now_us)/9*1e-6f;
	else
		Ts = (float)(0xFFFFFF - now_us + pid_current_timestamp)/9*1e-6f;
	pid_current_timestamp = now_us;
	if(Ts == 0 || Ts > 0.5) Ts = 1e-3f;
	...
	// proportional part 
	// u_p  = P *e(k)
	proportional = pid_current_P * error;
	// Tustin transform of the integral part
	// u_ik = u_ik_1  + I*Ts/2*(ek + ek_1)
	integral = integral_current_prev + pid_current_I*Ts*0.5*(error + error_current_prev);
	// antiwindup - limit the output
	integral = _constrain(integral, -voltage_limit, voltage_limit);
	...
	// limit the acceleration by ramping the output
	output_rate = (output - output_current_prev)/Ts;
	if(output_rate > output_current_ramp)output = output_current_prev + output_current_ramp*Ts;
	else if(output_rate < -output_current_ramp)output = output_current_prev - output_current_ramp*Ts;
	
	// saving for the next pass
	integral_current_prev = integral;
	output_current_prev = output;
	error_current_prev = error;
